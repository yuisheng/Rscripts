...
for(i in 1:length(modelList)){
  ...
  for(rcp in rcpList){
    filepath <- paste(fdir,rcp,"/","tas_Amon_",modelname,"_",rcp,"_1861-2099_r1i1p1_global-cor.nc",sep="")
    ...
    for(month in 1:nmonths){
      #Get the data by year
      rcp_wsl.data <- get.var.ncdf(rcp_wsl.file, "tas", count=c(-1,-1,1), start=c(1,1,month))
  ...
  write.table(rcp_wsl.monthly,paste(modelname,"r1i1p1","gye",".txt",sep="_"),row.names=FALSE, col.names=FALSE)
