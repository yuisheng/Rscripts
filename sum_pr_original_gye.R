...
#Calculate DAYMET
daymet.monthly <- 0
daymet.file <- open.ncdf(paste(fdir.daymet, "prcp_gye_1980-2012_daily_latlon_monthly-gye_shift.nc", sep=""))
for(month in 1:396){
  #Get the data by year
  daymet.data <- get.var.ncdf(daymet.file, "prcp", count=c(-1,-1,1), start=c(1,1,month))
...
for(i in 1:length(modelList)){
  ...
  for(rcp in rcpList){    
    filepath <- paste(fdir,rcp,"-wsl/","pr_Amon_",modelname,"_",rcp,"_r1i1p1_1861-2099-gye-mm.nc",sep="")
    print(filepath)
    ...
    for(month in 1:nmonths){
      #Get the data by year
      rcp_ori.data <- get.var.ncdf(rcp_ori.file, "pr", count=c(-1,-1,1), start=c(1,1,month))
